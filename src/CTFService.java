...
public class CTFService
{
    ...
    /** The voter list */
    private Hashtable m_voterList = new Hashtable();
    
    /** The candidate list */
    private Vector m_candidateList = new Vector();
    
    private boolean m_listening = false;

    /**
     * Executes an instance of the CTFService.
     */    
    ...
    /**
     * Starts the CTFService by adding the Security provider, reading in all configuration
     ...
     * that are requesting communication of data from the CTFService.
     */
        ...
        public void run()
        {
            try 
            {
                ...
                else if (request.equals("RESULTS"))
                {
                    ...
                    Vector electionResults = new Vector();
                    for(int i = 0; i < numCandidates; i++) 
                    {
                        String currentCandidateId = (String)m_candidateList.elementAt(i);
                        int currentCandidateVoteCount = 0;
                        Collection votes = m_voterList.values();
                        Iterator voteIterator = votes.iterator();
